...
apt-get install -qq --allow-downgrades \
  python3-pip \
  python3-dev \
...
cd /
chmod +x /Chatbot_8/scripts/start.sh /Chatbot_8/scripts/train.sh >/dev/null

echo -e "${Whi}<<Configuring Chatbot DB & running training>>${RCol}"
cd Chatbot_8 || exit
./scripts/train.sh
